...
object TagGen {
  ...
  def rootTemplate(moduleName: String, fullyQualifiedPath: String, contents: String): String =
  ...
  def extraTemplate(moduleName: String, fullyQualifiedPath: String, contents: String): String =
    s"""package $fullyQualifiedPath
    |
    |import tyrian.Html.text
    |import scala.annotation.targetName
  ...
  def gen(moduleName: String, fullyQualifiedPath: String, sourceManagedDir: File): Seq[File] = {
    ...
    val baseFiles = allTags.map { tagList =>
      val contents: String =
        tagList.tags.map(genTag).mkString

      val file: File =
        sourceManagedDir / (tagList.namespace + ".scala")

      val newContents: String =
        extraTemplate(tagList.namespace, fullyQualifiedPath, contents)
    ...
    val rootContents: String =
      rootTags.map(genTag).mkString

    val file: File =
      sourceManagedDir / (moduleName + ".scala")

    val newContents: String =
      rootTemplate(moduleName, fullyQualifiedPath, rootContents)
  ...
  def rootTags: List[TagType] =
    ...
    def svgTagList  = TagList(svgTags, "SVG")

    def allTags: List[TagList] = List(svgTagList)
}
