...
sealed trait Outcome[+A] derives CanEqual:
  ...
  def mapActions(f: Action => Action): Outcome[A]
...
object Outcome:
  ...
  final case class Result[+A](state: A, actions: Batch[Action]) extends Outcome[A] {
    ...
    def map[B](f: A => B): Outcome[B] =
      Outcome(f(state), actions)

    def mapActions(f: Action => Action): Outcome[A] =
      Outcome(state, actions.map(f))
  ...
  def join[A](faa: Outcome[Outcome[A]]): Outcome[A] =
    faa match {
      ...
      case Result(outcome, es) =>
        Outcome(outcome.unsafeGet, es ++ outcome.unsafeActions)
    }
