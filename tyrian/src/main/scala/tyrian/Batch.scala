...
object Batch:
  ...
  private[tyrian] final case class Combine[A](batch1: Batch[A], batch2: Batch[A]) extends Batch[A]:
  ...
  private[tyrian] final case class Wrapped[A](values: js.Array[A]) extends Batch[A]:
