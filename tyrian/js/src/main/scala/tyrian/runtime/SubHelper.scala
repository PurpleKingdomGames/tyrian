...
object SubHelper:
  ...
  def flatten[F[_]: Async, Msg](sub: Sub[F, Msg]): List[Sub.Observe[F, _, Msg]] =
    @tailrec
  ...
  def toRun[F[_]: Async, Msg](newSubs: List[Sub.Observe[F, _, Msg]], callback: Msg => Unit): List[F[SubToRun[F]]] =
